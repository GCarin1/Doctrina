...
test("every tracked source file in this repository has an owning capability", () => {
  ...
  const orphans = tracked.filter(
    (f) => rankCapabilitiesByDiff(repoRoot, [f], { limit: 1 }).length === 0);
  assert.deepEqual(orphans, [], "every file is claimed by the spec that owns it");
