...
flowchart TD
    subgraph BOOT["Bootstrap — uma vez por projeto"]
        ...
        specnew["doctrina spec new (cap)<br/>spec EARS + Realizes: + Depends on:"]
        intentadd["doctrina intent add<br/>novo anchor [SC] pós-intake"]
        init --> intake --> product --> specnew
        intentadd -. "a intenção evolui" .-> product
    end

    subgraph LOOP["Loop de change — uma vez por tarefa (conduzido pelo agente)"]
        direction TB
        work["doctrina work 'prompt'<br/>--capability esqueletiza o delta · --from-diff · --chore · --quiet · --resume"]
...
- `doctrina intent add "<texto>"` / `intent list` — anexa um novo anchor `[SC]`
  ao product.md quando a intenção evolui pós-intake, para capabilities novas
  terem o que `Realizes:` em vez de caírem em `n/a`.
...
- `doctrina prime` — o primer de sessão: gates, regras, trabalho aberto e
  ...
  `prime` e `handoff` são **vistas de um único snapshot** — os mesmos
  bytes de `doctrina status --view prime|handoff`, ao lado do `--view
  report` (o digest do período), renderizados a partir de uma só coleta da
  árvore, então não têm como reportar números diferentes.
